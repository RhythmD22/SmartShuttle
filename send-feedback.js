  ...
  try {
    ...
    if (serviceId && templateId && publicKey) {
      ...
      if (apiResponse.ok) {
        ...
        // If it's a 403 error, it's likely an authentication problem or API restriction
        if (apiResponse.status === 403) {
          console.error('EmailJS authentication error or API restriction - API calls are often disabled for non-browser applications');
          console.log('EmailJS does not allow server-side API calls in many configurations. Saving feedback locally as fallback...');
        }
      }
    } else {
